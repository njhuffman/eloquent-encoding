...
class ChessJEPA(nn.Module):
    ...
    def forward_online_stack(self, boards_bk: torch.Tensor) -> torch.Tensor:
        """boards_bk: (B, K, 8, 8, C) -> (B, K, D) online encoder on successor boards."""
        b, k, h, w, c = boards_bk.shape
        flat = boards_bk.reshape(b * k, h, w, c)
        z = self.encoder_online(flat)
        return z.reshape(b, k, -1)
