"""YAML model spec for rfp: defaults + per-stage deep merge (jepa3-style)."""
...
MODEL_CONFIGS_DIR = _REPO_ROOT / "rfp" / "model_configs"

RFP_ARCHITECTURE_ID = "rfp_residual_mixer"

_STAGE_FORBIDDEN_IN_DEFAULTS = ("sq_ce_label_smoothing", "elo_null_prob")
...
def _forbid_stage_keys_in_defaults(defaults: Any) -> None:
    ...
    if bad:
        raise ValueError(
            ...
            f"({', '.join(bad)}). Set them on each stages[i]."
        )
...
DEFAULTS: dict[str, Any] = {
    ...
    # Rolling / EMA train top1% for logging (see rfp.training_loop)
    "train_log_top1_window": 100,
    "train_log_top1_ema_beta": 0.99,
}
...
def load_model_spec(path: Path) -> dict[str, Any]:
    ...
    gfp_ck = raw.get("gfp_checkpoint")
    if not gfp_ck or not isinstance(gfp_ck, str):
        raise ValueError("gfp_checkpoint required (path to gfp stage .pt with head_state_dict)")
    raw["gfp_checkpoint"] = str(_resolve_repo_path(gfp_ck))

    for key in ("train_dataset_h5", "val_dataset_h5"):
        p = raw.get(key)
        if not p or not isinstance(p, str):
            raise ValueError(f"{key} required (path to rfp HDF5)")
    ...
    if not ckpt:
        raw["checkpoint_dir"] = str((_REPO_ROOT / "rfp_checkpoints" / raw["name"]).resolve())
    else:
    ...
    if str(arch["id"]) != RFP_ARCHITECTURE_ID:
        raise ValueError(f'architecture.id must be {RFP_ARCHITECTURE_ID!r} (got {arch.get("id")!r})')
    cfg = arch.get("config")
    ...
    for k in (
        ...
        "mixer_dim",
        "mixer_depth",
        "mixer_tokens_mlp_dim",
        "mixer_channels_mlp_dim",
        "mixer_dropout",
        "elo_num_buckets",
        "elo_embed_dim",
        "residual_hidden",
    ...
    cfg["history_len"] = int(cfg["history_len"])
    if cfg["history_len"] < 1:
        raise ValueError("architecture.config.history_len must be >= 1")
    cfg["mixer_dim"] = int(cfg["mixer_dim"])
    cfg["mixer_depth"] = int(cfg["mixer_depth"])
    cfg["mixer_tokens_mlp_dim"] = int(cfg["mixer_tokens_mlp_dim"])
    cfg["mixer_channels_mlp_dim"] = int(cfg["mixer_channels_mlp_dim"])
    cfg["mixer_dropout"] = float(cfg["mixer_dropout"])
    cfg["elo_num_buckets"] = int(cfg["elo_num_buckets"])
    cfg["elo_embed_dim"] = int(cfg["elo_embed_dim"])
    cfg["residual_hidden"] = int(cfg["residual_hidden"])
    cfg["residual_depth"] = int(cfg["residual_depth"])
    ...
    for i, st in enumerate(stages):
        ...
        if "elo_null_prob" not in st:
            raise KeyError(f"stages[{i}] must set elo_null_prob (per stage)")
        ep = float(st["elo_null_prob"])
        if not math.isfinite(ep) or ep < 0.0 or ep > 1.0:
            raise ValueError(f"stages[{i}].elo_null_prob must be in [0, 1] (got {st['elo_null_prob']!r})")
...
def resolve_training_config_for_stage(spec: dict[str, Any], stage_index: int) -> dict[str, Any]:
    ...
    ep = float(merged["elo_null_prob"])
    if not math.isfinite(ep) or ep < 0.0 or ep > 1.0:
        raise ValueError(f"elo_null_prob must be in [0, 1] (got {merged['elo_null_prob']!r})")
    ...
    tw = int(merged.get("train_log_top1_window", 100))
    if tw < 1:
        raise ValueError(f"train_log_top1_window must be >= 1 (got {tw!r})")
    merged["train_log_top1_window"] = tw

    eb = float(merged.get("train_log_top1_ema_beta", 0.99))
    if not math.isfinite(eb) or eb <= 0.0 or eb >= 1.0:
        raise ValueError(f"train_log_top1_ema_beta must be in (0, 1) (got {merged.get('train_log_top1_ema_beta')!r})")
    merged["train_log_top1_ema_beta"] = eb
...
def spec_path_for_model(model_name: str) -> Path:
    for ext in (".yaml", ".yml"):
        ...
        if p.is_file():
            return p
    raise FileNotFoundError(f"No rfp spec at {MODEL_CONFIGS_DIR / (model_name + '.yaml')}")
