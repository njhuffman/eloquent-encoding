...
def test_chess_jepa_v3_forward_smoke() -> None:
    ...
    assert z_hat.shape == (b, cfg["predictor_prefix_dims"])
...
def test_move_head_prefix_leak_zero_grad_in_prefix_slice_of_z() -> None:
    cfg = resolve_architecture_config(
        {
            "d_model": 64,
            "encoder_layers": 1,
    ...
    lf = m.forward_from_logits(z, move_head_prefix_leak=0.0)
    lf.sum().backward()
    p = int(cfg["predictor_prefix_dims"])
    assert z.grad is not None
    assert z.grad[:, :p].abs().max().item() == 0.0
    assert z.grad[:, p:].abs().max().item() > 0.0

    z2 = torch.randn(2, cfg["d_model"], requires_grad=True)
    lt = m.forward_to_logits(z2, torch.tensor([0, 1], dtype=torch.long), move_head_prefix_leak=0.0)
    lt.sum().backward()
    assert z2.grad is not None
    assert z2.grad[:, :p].abs().max().item() == 0.0
    assert z2.grad[:, p:].abs().max().item() > 0.0
