"""
Named model registry under embedding/artifacts: registry.json + per-name checkpoint.pt
"""
...
from embedding.config import ARTIFACTS_DIR, CHECKPOINT_BASENAME, REGISTRY_FILENAME

# Short memorable names (adjective-noun); expand freely.
_ADJECTIVES = (
...
def parse_arch_config_arg(arg: str | None) -> dict[str, Any]:
    """CLI: path to JSON file, or inline JSON object string."""
...
def register_model(
    *,
    ...
    """
    Write checkpoint to artifacts/<name>/checkpoint.pt and append registry.json entry.
    checkpoint_payload is the dict passed to torch.save.
    """
    root = repo_root or _repo_root()
