"""Paths for jepa2 per-stage metrics (single JSON per training stage)."""
...
def stage_metrics_json_path(checkpoint_dir: Path | str, model_name: str, stage: int) -> Path:
    """One JSON object per completed training stage: ``{checkpoint_dir}/metrics/{name}_stage_{stage}_metrics.json``."""
