"""
PGN → gfp slim HDF5 row streaming.

Keep sampling logic aligned with jepa3/packed_build_stream.py when that file changes.
"""
...
from jepa3.packed_board_codec import board_tensor_to_packed, legal_mask_float_to_u64
from move_predictor.encoding import move_to_from_to
from gfp.h5_io import DATASET_PACKED_PRE, GfpH5Writer
...
def _write_samples_gfp(
    ...
    *,
    ...
    if k < stratum.samples_per_game:
        raise RuntimeError(
            "internal error: _write_samples_gfp requires "
    ...
    for j in _sample_indices(rng, k, stratum.samples_per_game):
        ply, _stm, _elo, move = candidates[j]
...
def build_gfp_from_recipe(
    ...
    *,
    ...
    try:
        with GfpH5Writer(out_path) as writer:
        ...
        if out_path.exists():
            ...
            logger.error("GFP build failed; removed partial output %s: %s", out_path, e)
        else:
            logger.error("GFP build failed (no output file written): %s", e)
        raise
