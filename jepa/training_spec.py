"""
Load and validate Chess-JEPA training JSON specs (jepa/model_configs/).
"""
...
DEFAULT_TRAINING_SPEC: dict[str, Any] = {
    ...
    "training": {
        ...
        "ema_momentum": EMA_MOMENTUM_DEFAULT,
        "triplet_margin_alpha": TRIPLET_MARGIN_ALPHA_DEFAULT,
        "vicreg_var_coef": VICREG_VAR_COEF_DEFAULT,
...
def normalize_training_spec(
    ...
    *,
    ...
    tr["weight_decay"] = float(tr.get("weight_decay", WEIGHT_DECAY))
    tr["ema_momentum"] = float(tr.get("ema_momentum", EMA_MOMENTUM_DEFAULT))
    tr["triplet_margin_alpha"] = float(tr.get("triplet_margin_alpha", TRIPLET_MARGIN_ALPHA_DEFAULT))
    tr["vicreg_var_coef"] = float(tr.get("vicreg_var_coef", VICREG_VAR_COEF_DEFAULT))
    tr["vicreg_std_target"] = float(tr.get("vicreg_std_target", VICREG_STD_TARGET))
