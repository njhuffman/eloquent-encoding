"""Resolve next training stage and validate model names for the dashboard runner."""
...
from jepa.model_spec import MODEL_CONFIGS_DIR
...
def next_missing_stage(spec: dict) -> tuple[int | None, str]:
    """
    Returns (stage, message). stage is None if fully complete or blocked (missing earlier ckpt).
    """
    name = spec["name"]
