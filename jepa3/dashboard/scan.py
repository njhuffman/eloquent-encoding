"""Discover jepa3 model specs, checkpoint stages, and per-stage metrics JSON."""
...
from jepa3.metrics_paths import (
    model_profile_json_path,
    stage_benchmarks_json_path,
    stage_metrics_json_path,
)
from jepa3.model_spec import MODEL_CONFIGS_DIR, load_model_spec
...
def load_spec_by_model_name(name: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"No jepa3 spec for model {name!r}")
...
def read_curves_payload_for_stage(spec: dict[str, Any], stage: int) -> dict[str, Any]:
    """
    Compatibility shape for the dashboard: ``epochs`` is a one-element list holding
    the per-stage metrics object (no per-epoch time series).
    """
    name = spec["name"]
    ckpt_dir = Path(spec["checkpoint_dir"])
    mpath = stage_metrics_json_path(ckpt_dir, name, stage)
    obj = read_stage_metrics_json(mpath)
    if obj:
        return {
            "model": name,
            "stage": stage,
...
def sparse_stage_points_from_checkpoints(spec: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for s in range(1, len(spec["stages"]) + 1):
        ...
        tm = summ.get("train_meta") or {}
        if th.get("init_only") or tm.get("init_only"):
            continue
        points.append(
            {
                "stage": s,
